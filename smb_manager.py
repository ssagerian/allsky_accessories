...
class DeviceTSL2560:
    ...
    def get_last_lux(self):
        ...
        Lux = 0
        """
        if self.channel1.last_read > 0:
            cratio = self.channel0.last_read / self.channel1.last_read
            if 0 < cratio <= 0.5:
                lux = (0.0304 * self.channel0.last_read) - (0.062 * self.channel0.last_read) * pow(cratio, 1.40)
            elif 0.50 < cratio <= 0.61:
                lux = (0.0224 * self.channel0.last_read) - (0.031 * self.channel1.last_read)
            elif 0.61 < cratio <= 0.80:
                lux = (0.0128 * self.channel0.last_read) - (0.0153 * self.channel1.last_read)
            elif 0.80 < cratio <= 1.30:
                lux = (0.00146 * self.channel0.last_read) - (0.00112 * self.channel1.last_read)
            else:
                lux = 0
            return lux
